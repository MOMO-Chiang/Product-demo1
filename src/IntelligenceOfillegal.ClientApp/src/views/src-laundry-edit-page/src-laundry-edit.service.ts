...
export const fetchExternalIntelligence = (id: string) => {
  const resp = AdminHttpRequest.get<any>(`/api/intelligenceCase35/${id}`);
...
export const editCaseDistributeById = async (editModel: CaseDistributeEditModel) =>
  AdminHttpRequest.post(`/api/intelligenceCase35/case-distribute`, editModel);
...
export const fetchExternalIntelligenceOption = (id: string) => {
  const resp = AdminHttpRequest.post<any>(`/api/intelligenceCase35/option`, id);
