...
import { fetchRptEconomyIntelligenceList, exportExcel } from '../rpt-economy-intelligence.service';
...
export const PageContextProvider: FC<PageContextProviderProps> = ({ children }) => {
  ...
  const {
    ...
    setSortedModel,
  } = useAjaxDataGrid<RptEconomyIntelligenceModel, number>({ pkField: 'itlgSrcFileNo' });

  /** 搜尋分頁列表資料 */
  const fetchPaginatedRptEconomyIntelligenceList = async (params: {
    ...
    try {
      // 取得資料
      const respData = await fetchRptEconomyIntelligenceList({
        page,
        pageSize,
        sortedColumn,
        sortedType,
  ...
  const handleSearchFormSubmit = async (searchFormData: SearchCardForm) => {
    await fetchPaginatedRptEconomyIntelligenceList({
      searchFormData,
      page: DEFAULT_PAGE,
