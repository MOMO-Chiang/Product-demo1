...
export const fetchExternalIntelligence = (id: string) => {
  const resp = AdminHttpRequest.get<any>(`/api/intelligenceCase18/${id}`);
...
export const editCaseDistributeById = async (editModel: CaseDistributeEditModel) =>
  AdminHttpRequest.post(`/api/intelligenceCase18/case-distribute`, editModel);
...
export const fetchExternalIntelligenceOption = (id: string) => {
  const resp = AdminHttpRequest.post<any>(`/api/intelligenceCase18/option`, id);
