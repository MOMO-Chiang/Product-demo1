...
import { SearchCardForm, RptLaundryIntelligenceModel } from '../types';
import {
  PageChangeHandler,
...
import { fetchRptLaundryItlgList, exportExcel } from '../rpt-laundry-intelligence.service';
...
export const PageContextProvider: FC<PageContextProviderProps> = ({ children }) => {
  ...
  const {
    ...
    setSortedModel,
  } = useAjaxDataGrid<RptLaundryIntelligenceModel, number>({ pkField: 'intelligenceNo' });

  /** 搜尋分頁列表資料 */
  const fetchPaginatedRptLaundryItlgList = async (params: {
    ...
    try {
      // 取得資料
      const respData = await fetchRptLaundryItlgList({
        page,
        pageSize,
        sortedColumn,
        sortedType,
  ...
  const handleSearchFormSubmit = async (searchFormData: SearchCardForm) => {
    await fetchPaginatedRptLaundryItlgList({
      searchFormData,
      page: DEFAULT_PAGE,
