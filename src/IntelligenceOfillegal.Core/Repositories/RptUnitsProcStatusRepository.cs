...
{
   public class RptUnitsProcStatusRepository : RepositoryBase<CaseManagement>, IRptUnitsProcStatusRepository
    {
        public RptUnitsProcStatusRepository(IDbConnectionFactory dbConnectionFactory, string connectionName)
            : base(dbConnectionFactory, connectionName)
        {
        }

        public Tuple<IEnumerable<RptUnitsProcStatusListModel>, int> SearchPaginated(RptUnitsProcStatusListSearchModel entity, PaginationWithSortedQueryModel paginated)
        {
            string sqlSelect = $@"
...
_data AS (
	...
	LEFT JOIN MergeCase ON MergeCase.ItlgSrcReportUnitCode = MUC.UnitCode
            ...
            IEnumerable<RptUnitsProcStatusListModel> results = result.Read<RptUnitsProcStatusListModel>();
            int totalCount = result.ReadFirst<int>();
            Connection.Close();
            return new Tuple<IEnumerable<RptUnitsProcStatusListModel>, int>(results, totalCount);
        }
